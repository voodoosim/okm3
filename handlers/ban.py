...
from utils.logger import log_ban
...
async def ban_user_cmd(message: types.Message, bot: Bot) -> None:
    """사용자 차단 명령어 (.벤)."""
    logger.info(
        "ban_cmd_triggered",
    ...
    try:
        if not message.from_user:
            logger.error("ban_cmd_error", error="No user information")
        ...
        if "user_ids" in user_info and user_info["user_ids"]:
            ...
            results = []
            tasks = [
                process_ban(message, bot, target_id, None, user_info["reason"] or "")
            ...
            if failed_users:
                message_text.append(f"차단 실패: {', '.join(failed_users)}")
            ...
            for group_id, result in zip(groups.keys(), results):
                if isinstance(result, Exception):
                    logger.error(
                        "group_ban_failed", group_id=group_id, error=str(result)
                    )
            return
        ...
        result = await process_ban(message, bot, target_id, target_username, reason or "")
        if result:
            # 단일 차단 알림
            ...
            try:
                await message.reply(ban_message, parse_mode="HTML")
                logger.info("reply_sent", chat_id=chat_id, message=ban_message)
            except Exception as e:
                logger.error("reply_failed", chat_id=chat_id, error=str(e))

            # 단일 로그 전송
            logger.info("calling_log_ban_single", chat_id=chat_id, target_id=target_id)
            await log_ban(
                bot,
                [(result[0], target_id)],
                message.from_user.id,
        ...
        logger.error("ban_cmd_value_error", chat_id=message.chat.id, error=str(e))
        await message.reply(f"입력 오류: {str(e)}")
    except Exception as e:
        logger.error("ban_cmd_error", chat_id=message.chat.id, error=str(e))
...
async def process_ban(
    ...
    logger.info("process_ban_attempt", chat_id=chat_id, target_id=target_id)

    if not message.from_user:
        logger.error("process_ban_error", chat_id=chat_id, error="No user information")
        return None

    if target_id == message.from_user.id or target_id == bot.id:
        logger.error("process_ban_error", chat_id=chat_id, error="Cannot ban self or bot")
        return None
    ...
    try:
        ...
        if isinstance(chat_member, (types.ChatMemberOwner, types.ChatMemberAdministrator)):
            logger.error("process_ban_error", chat_id=chat_id, error="Cannot ban admin")
            return None

        user = chat_member.user
        target_username = user.username or f"<b>{user.first_name} {user.last_name}</b>".strip() or "<b>Nickname</b>"
        await bot.ban_chat_member(chat_id, target_id)
        logger.info("ban_chat_member_success", chat_id=chat_id, target_id=target_id)
        ...
        logger.error("process_ban_error", chat_id=chat_id, target_id=target_id, error=str(e))
        raise
